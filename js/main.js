...
class TodoManager {
  ...
  addTodo(task, dueDate, description, SubTask) {
    const newTodo = {
      id: this.getRandomId(),
      ...
      SubTask: this.todoItemFormatter.formatSubTask(SubTask)
    };
...
class UIManager {
  ...
  handleAddTodo() {
    ...
    if (task === "") {
      ...
      const newTodo = this.todoManager.addTodo(task, dueDate, description, SubTask);
      this.showAllTodos();
    ...
    todos.forEach((todo) => {
      this.todosListBody.innerHTML += `
          <tr class="todo-item" data-id="${todo.id}">
            ...
            <td>${this.todoItemFormatter.formatSubTask(todo.SubTask)}</td>   
